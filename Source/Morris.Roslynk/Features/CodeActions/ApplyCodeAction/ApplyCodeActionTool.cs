...
public sealed class ApplyCodeActionTool
{
	...
	[Description(
		...
		""")]
	public async Task<ApplyCodeActionResponse> ApplyCodeAction(
		...
		CancellationToken cancellationToken = default)
	{
		if (!CodeActionService.TryDecodeId(actionId, out ActionRef actionRef))
			return new ApplyCodeActionResponse(Applied: false, [], null, "The actionId could not be decoded; get a fresh one from get_code_actions.");
		...
		if (document is null)
			return new ApplyCodeActionResponse(Applied: false, [], actionRef.Key, $"'{actionRef.DocumentPath}' is no longer a solution document.");

		Solution? changed = await CodeActionService.ComputeChangedSolutionAsync(document, actionRef, cancellationToken);
		if (changed is null)
			return new ApplyCodeActionResponse(Applied: false, [], actionRef.Key, "The action is no longer available — the code may have changed. Re-run get_code_actions.");

		if (checkOnly)
			return new ApplyCodeActionResponse(Applied: false, ApplyPipeline.GetChangedFilePaths(solution, changed), actionRef.Key, "Preview only; nothing was written.");

		IReadOnlyList<string> files = await ApplyPipeline.ApplyAsync(instance, changed, cancellationToken);
		return new ApplyCodeActionResponse(Applied: true, files, actionRef.Key, Message: null);
	}
}
